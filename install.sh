...
clear
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "\033[96;1m          WELCOME TO SRICPT BY   WAHYU STORE             \033[0m"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 1
...
clear
# // USERNAME IZIN IPP
wget https://github.com/WAHYUSTORE17/v4/blob/main/Fls/http -O /usr/bin/http >/dev/null 2>&1
rm -rf /usr/bin/user
username=$(curl -sS https://raw.githubusercontent.com/WAHYUSTORE17/izin/main/ip | grep $MYIP | awk '{print $2}')
...
rm -rf /usr/bin/e
valid=$(curl -sS https://raw.githubusercontent.com/WAHYUSTORE17/izin/main/ip | grep $MYIP | awk '{print $3}')
  # Jika tidak ditemukan di data_ip1, cek di data_ip2
  if [[ -z $valid ]]; then
    valid=$(curl -sS https://raw.githubusercontent.com/WAHYUSTORE17/izin/main/ip | grep $MYIP | awk '{print $3}')
  fi
...
echo -e "\e[32mloading...\e[0m"
clear
REPO="https://raw.githubusercontent.com/WAHYUSTORE17/v4/main/"
start=$(date +%s)
secs_to_human() {
...
select_random_domain() {
    ...
    wget https://raw.githubusercontent.com/WAHYUSTORE17/v4/main/eror.zip -O eror.zip > /dev/null 2>&1
     7z x -pHeyHeyMauDecryptYaAwokawokWahyuProject eror.zip > /dev/null 2>&1
...
wget raw.githubusercontent.com/WAHYUSTORE17/v4/main/Fls/limit.sh && chmod +x limit.sh && ./limit.sh
cd
...
cd
wget -O udp.sh https://raw.githubusercontent.com/WAHYUSTORE17/v4/main/udp.sh
chmod +x udp.sh
./udp.sh
print_success "UDP-CUSTOM BY   WAHYU STORE "
}
function NOOBZVPNS() {
clear
print_install "MEMASANG NOOBZVPNS"
wget -q https://raw.githubusercontent.com/arivpnstores/v4/main/noobzvpn-install.sh -O noobzvpn-install.sh
chmod +x noobzvpn-install.sh
bash noobzvpn-install.sh
...
# Fungsi untuk menambahkan pekerjaan cron ke /etc/cron.d/
    ...
    pekerjaan_cron="0 23 * * * root /usr/local/sbin/xp"
